...
//  rest
//
//  Created by kalsky_953982 on 2/13/25.
//
...
func writeToFile(label: Int64, Matrix: [[Int]], fileName: String){
    ...
    let description = "\(label)," + Matrix.description.replacingOccurrences(of: "[", with: "").replacingOccurrences(of: "]", with: "") + "\n"
    
    guard let data = description.data(using: .utf8) else{print("unable to convert"); return}
...
func removeLast(){
    let directoryURL = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0]
    let dataURL = URL(fileURLWithPath: "Sampling Data", relativeTo: directoryURL).appendingPathExtension("csv")
    let removeFileURL = URL(fileURLWithPath: "remove", relativeTo: directoryURL).appendingPathExtension("csv")
    
    do{
        let data = try Data(contentsOf: dataURL)
        if let stringData = String(data: data, encoding: .utf8){
            let fileEntreeList = stringData.split(separator: "\n")
            let fileHandle = try FileHandle(forWritingTo: removeFileURL)
            try fileHandle.seekToEnd()
            //index excluding header row
            if let indexData = "\(fileEntreeList.count-2), ".data(using: .utf8){
                try fileHandle.write(contentsOf: indexData)
            }
        }
    }catch{
        print(error.localizedDescription)
