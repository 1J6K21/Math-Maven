...
func getNumber(line: Line, canvasSize: Int, sample: Bool) async -> ((Int64, Double), (Int64, Double)){
    ...
    do{
        ...
        let model = try DigitClassifierV7(configuration: config)
...
func getMatrix(line: Line, canvasSize: CGFloat, collectData: Bool = false) async -> [[Int]] {
    ...
    if collectData{
        writeToFile(label: testingLabel, Matrix: Matrix, fileName: "Sampling Data")
        writeToFile(label: testingLabel, Matrix: uncenteredMatrix, fileName: "Sampling Data Uncentered")
    }
    return Matrix
