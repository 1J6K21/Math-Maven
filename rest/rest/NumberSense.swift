...
//                    Spacer(minLength: 100)
                    ...
                    HStack(alignment: .center){
                        ...
                        Text(String(format: "Accuracy %d%%", (100*correctCount)/max(1,questionNum)))
                            .font(.title2)
                    ...
                    if !showHelp{
                        Canvas{ context, size in
                            ...
                            //other strokes
                            for line in lines {
                                drawLine(context: context, line: line, color: Color.CS_6)
                            }
                        }
                        .shadow(radius: 1.5)
                        .frame(width: 300, height: 300)
        ...
        .onChange(of: answeredWrong, {
            finalTime = time
            time = 0
        })
        .onChange(of: answeredRight, {
            finalTime = time
            time = 0
        })
        .onChange(of: next, { _,new in
            if new{
                work = ""
